...
BEGIN;
	...
	(
		...
		(
			...
			SELECT '[host_name]', 36
			UNION ALL
			SELECT '[client_net_address]', 37
			UNION ALL
			SELECT '[client_tcp_port]', 38
			UNION ALL
			SELECT '[login_name]', 39
			UNION ALL
			SELECT '[database_name]', 40
			UNION ALL
			SELECT '[program_name]', 41
			UNION ALL
			SELECT '[additional_info]', 42
			WHERE
				@get_additional_info = 1
			UNION ALL
			SELECT '[start_time]', 43
			UNION ALL
			SELECT '[login_time]', 44
			UNION ALL
			SELECT '[request_id]', 45
			UNION ALL
			SELECT '[collection_time]', 46
		) AS x ON 
	...
	(
		...
		client_net_address NVARCHAR(4000) NULL,
		client_tcp_port  NVARCHAR(4000) NULL,
					...
					(
						...
						(
							SELECT TOP(@i)
								...
								MAX(CONVERT(NVARCHAR(128), RTRIM(conn.client_net_address)) COLLATE SQL_Latin1_General_CP1_CI_AS) AS client_net_address,
								MAX(CONVERT(NVARCHAR(128), RTRIM(conn.client_tcp_port)) COLLATE SQL_Latin1_General_CP1_CI_AS) AS client_tcp_port,
							...
							LEFT JOIN SYS.DM_EXEC_CONNECTIONS AS conn ON
								conn.SESSION_ID  = blk.session_id
								OR
								(
									blk.session_id = 0
									AND @blocker = 0
...
/**/
				CASE
					WHEN 
						@output_column_list LIKE '%|[client_net_address|]%' ESCAPE '|' 
						AND @recursion = 1
							THEN 
								'x.client_net_address '
					ELSE 
						''''' '
				END + 
				...
				CASE
					WHEN 
						@output_column_list LIKE '%|[client_tcp_port|]%' ESCAPE '|' 
						AND @recursion = 1
							THEN 
								'x.client_tcp_port '
					ELSE 
